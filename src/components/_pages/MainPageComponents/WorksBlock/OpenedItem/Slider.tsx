    ...
    return (
        <div className={s.slider} ref={sliderRef}>
            <button
                ...
                onClick={setPrevHandler}
            >
                <IconSelector icon="arrowIcon" />
            </button>
            <button
                className={s.right}
            ...
            <div
                className={`${s.counter} ${currentImage > 1 ? s.active : ""}`}
            >{`${currentImage}/${images.length}`}</div>
            <div
                className={s.imagesWrapper}
                style={{ transform: `translateX(-${sliderScroll}px)` }}
            >
                {images &&
